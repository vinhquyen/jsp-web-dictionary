...
--
-- Estructura para la vista `v_word`
--
DROP TABLE IF EXISTS `v_word`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_word` AS select `w`.`id` AS `id`,`wd`.`n_def` AS `n_def`,`w`.`term` AS `term`,`w`.`morf` AS `morf`,`wd`.`definition` AS `definition`,`w`.`validated` AS `validated`,`w`.`relation` AS `relation` from (`word` `w` join `word_definition` `wd`) where (`w`.`id` = `wd`.`id`) order by `w`.`id`,`wd`.`n_def`;
