...
case 'register':
	...
	if ($user_email == '') {
			$no_mail = true;
			$user_email = 'user@facebook.com'; //Es un usuario FB sin mail
...
<div id="login"> 
	...
	<?php 
		//Fixme -- Si no hay email, no le decimos que le hemos enviado passwd
	if ($user_email != '') {
		printf(__('Password: %s'), '<strong>' . __('emailed to you') . '</strong>') 
	}
	else
		printf("Remeber you need to login into FaceBook to send comments!");
		?> <br />
...
<body>
<div id="login">
<h1><a href="http://chkserver.no-ip.org/wordpress/">CataVino</a></h1>
<h2><?php _e('Register for CataVino') ?></h2>
<?php if ( isset($errors) ) : ?>
<div class="error">
	<ul>
	<?php
...
<form method="post" action="wp-register.php" id="registerform">
	...
	<p><label for="user_email"><?php _e('E-mail: (optional)**') ?></label><br /> <input type="text" name="user_email" id="user_email" size="25" maxlength="100" value="<?php echo attribute_escape($user_email); ?>" /></p>
...
<p>** If any e-mail is introduced you cannot enjoy all the functionalities!<br/>
	You could send Comments using FaceBook, but you could not login in 
	the CataVino site or other WordPress blogs.
</p>
<ul>
	<li><a href="javascript:window.opener='x';window.close();" title="<?php _e('Close Window') ?>">&laquo; <?php _e('Close') ?> &raquo;</a></li>
</ul>
</div>
