...
/** 
	PXC - Funcion que devuelve string con datos ranking 
*/
	function getRatingRSS2() {
		global $id, $wpdb, $table_prefix;
		$table_name = $table_prefix . "psr_post";

		$votes  = (string) $wpdb->get_var("SELECT votes  FROM {$table_name} WHERE id={$id}");
		$points = (string) $wpdb->get_var("SELECT points FROM {$table_name} WHERE id={$id}");
		
		$res = "<votes>$votes</votes><points>$points</points>";
		return $res;
		}
	/**
...
//				$html .= ' <span class="position">' . $row->position . '</span>';
	...
	function css()
	{
		...
		.PSR_stars {
		  height: 40px;		/* Escala div puntuciones */
		  overflow: hidden;
		  padding: 0;
		  margin: 0;
		}
		* html .PSR_stars .star {
		...
		.PSR_no_star, .PSR_half_star, .PSR_full_star, .PSR_no_voting_star, .PSR_half_voting_star, .PSR_full_voting_star {
		  display: block;
		  float: left;
		  width: 22px;	/* Escala espacio ocupado imagenes */
		  height: 40px;
		  text-indent: -1000em;
		  text-align: left;
		...
		.PSR_no_star {
		  background-image: url(" . $home . "/wp-content/plugins/post-star-rating/img/wine/no_star.gif);
		}
		.PSR_full_star {
		  background-image: url(" . $home . "/wp-content/plugins/post-star-rating/img/wine/full_star.gif);
		}
		.PSR_half_star {
		  background-image: url(" . $home . "/wp-content/plugins/post-star-rating/img/wine/half_star.gif);
		}
		.PSR_no_voting_star {
		  background-image: url(" . $home . "/wp-content/plugins/post-star-rating/img/wine/no_star.gif);
		}
		.PSR_full_voting_star {
		  background-image: url(" . $home . "/wp-content/plugins/post-star-rating/img/wine/full_voting_star.gif);
		}
		.PSR_half_voting_star {
		  background-image: url(" . $home . "/wp-content/plugins/post-star-rating/img/wine/half_voting_star.gif);
		}
		.PSR_votes {
			padding-left: .5em;
		}
